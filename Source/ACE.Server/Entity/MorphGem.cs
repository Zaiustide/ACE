...
namespace ACE.Server.Entity
{
    public class MorphGem
    {
        ...
        public const uint MorphGemCreatureRandom = 480610;
        ...
        public const uint MorphGemCreatureSlayerRating = 600038;
        public const uint MorphGemCreatureResistRating = 600037;
        ...
        public static HashSet<uint> MorphGems = new HashSet<uint>()
        {
            ...
            MorphGemCreatureSlayerRating,
            MorphGemCreatureResistRating
        };
        ...
        public static readonly HashSet<uint> WorldBossJewelry = new HashSet<uint>()
        {
            490000,
            490001,
            490002,
        ...
        public static void ApplyMorphGem(Player player, WorldObject source, WorldObject target)
        {
            try
            {
                ...
                switch (source.WeenieClassId)
                {
                    ...
                    #region MorphGemCreatureRandom
                    case MorphGemCreatureRandom:
                        ...
                        else if (target.GearCreatureResistType != CreatureType.Invalid && target.GearCreatureResistRating > 0)
                        {
                        ...
                        {
                            playerMsg = "This gem can only be used on loot generated weapons or casters with a Slayer from the Tinkering Lottery or armor, jewelry or undies with a Creature Resist or Creature Slayer Rating.";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                        ...
                        AddMorphGemLog(target, MorphGemCreatureRandom);
                        break;

                    #endregion MorphGemCreatureRandom

                    #region MorphGemCreatureResistRating
                    case MorphGemCreatureResistRating:

                        if (target.GearCreatureResistRating >= 3)
                        {
                            playerMsg = $"Your {target.NameWithMaterial} has the maximum Creature Resist Rating of {target.GearCreatureResistRating} and cannot be further improved.";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                        ...
                        {
                            var oldRating = target.GearCreatureResistRating;
                            var newRating = ThreadSafeRandom.Next(1, 3);
                            target.GearCreatureResistRating = newRating;

                            if (newRating > oldRating)
                            {
                                playerMsg = $"You have successfully used the {source.Name} to upgrade your {target.NameWithMaterial} with {target.GearCreatureResistType.ToDisplayString()} Resist Rating {newRating}!";
                            }
                            else if (newRating == oldRating)
                            {
                                playerMsg = $"The {source.Name} has failed to upgrade your {target.NameWithMaterial} with its {target.GearCreatureResistType.ToDisplayString()} Resist Rating of {newRating} remaining unchanged.";
                            }
                            else
                            {
                                playerMsg = $"The {source.Name} has failed and has damaged your {target.NameWithMaterial} resulting in a new {target.GearCreatureResistType.ToDisplayString()} Resist Rating of {newRating}.";
                            }                            
                        }
                        else
                        {
                        ...
                        player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                        AddMorphGemLog(target, MorphGemCreatureResistRating);
                        break;

                    #endregion MorphGemCreatureResistRating

                    #region MorphGemCreatureSlayerRating
                    case MorphGemCreatureSlayerRating:

                        if(target.GearCreatureSlayerRating >= 3)
                        {
                            playerMsg = $"Your {target.NameWithMaterial} has the maximum Creature Slayer Rating of {target.GearCreatureSlayerRating} and cannot be further improved.";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }

                        if (target.GearCreatureSlayerType != CreatureType.Invalid && target.GearCreatureSlayerRating > 0)
                        {
                            var oldRating = target.GearCreatureSlayerRating;
                            var newRating = ThreadSafeRandom.Next(1, 3);
                            target.GearCreatureSlayerRating = newRating;

                            if (newRating > oldRating)
                            {
                                playerMsg = $"You have successfully used the {source.Name} to upgrade your {target.NameWithMaterial} with {target.GearCreatureSlayerType.ToDisplayString()} Slayer Rating {newRating}!";
                            }
                            else if (newRating == oldRating)
                            {
                                playerMsg = $"The {source.Name} has failed to upgrade your {target.NameWithMaterial} with its {target.GearCreatureSlayerType.ToDisplayString()} Slayer Rating of {newRating} remaining unchanged.";
                            }
                            else
                            {
                                playerMsg = $"The {source.Name} has failed and has damaged your {target.NameWithMaterial} resulting in a new {target.GearCreatureSlayerType.ToDisplayString()} Slayer Rating of {newRating}.";
                            }
                        }                       
                        else
                        {
                            playerMsg = "This gem can only be used on loot generated armor, jewelry or undies with a Creature Slayer Rating.";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                        ...
                        player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                        AddMorphGemLog(target, MorphGemCreatureSlayerRating);
                        break;

                    #endregion MorphGemCreatureSlayerRating
                    ...
                    case MorphGemCDR:
                        ...
                        if (!(target.ItemType == ItemType.Armor || target.ItemType == ItemType.Jewelry || target.ItemType == ItemType.Clothing) || target.IsShield)
                        {
                            playerMsg = "This gem can only be used on armor, jewelry and undies";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                        ...
                        if ((target.ItemType == ItemType.Armor || target.ItemType == ItemType.Clothing) && target.ArmorLevel > 0)
                        {
                            if (target.GearCritDamageResist >= 3)
                            {
                            ...
                            if (bracketRoll > 0.999f)
                            {
                                newCdrRating = 3;
                            }
                            else if (bracketRoll > 0.995f)
                            {
                                newCdrRating = ThreadSafeRandom.Next(2, 3);
                            }
                            else if (bracketRoll > 0.99f)
                            {
                                newCdrRating = ThreadSafeRandom.Next(1, 3);
                            }
                            else if (bracketRoll > 0.5f)
                            {
                                newCdrRating = ThreadSafeRandom.Next(1, 2);
                            }
                            else
                            {
                                newCdrRating = 1;
                        ...
                        //Jewelry, Undies or Cloak
                        else if (target.ItemType == ItemType.Jewelry ||
                            ...
                            (target.ValidLocations?.HasFlag(EquipMask.Cloak) ?? false))
                        {
                            if (target.GearCritDamageResist >= 3)
                            {
                                playerMsg = $"Your {target.NameWithMaterial} already has a Critical Damage Resist Rating of +{target.GearCritDamageResist} and cannot be further upgraded.";
                                player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                                player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                                return;
                            }
                            
                            var cdrBracketRoll = ThreadSafeRandom.Next(0.0f, 1.0f);

                            if (cdrBracketRoll > 0.999f)
                            {
                                newCdrRating = 3;
                            }
                            else if (cdrBracketRoll > 0.9f)
                            {
                                newCdrRating = ThreadSafeRandom.Next(2, 3);
                            }
                            else if (cdrBracketRoll > 0.5f)
                            {
                                newCdrRating = ThreadSafeRandom.Next(1, 2);
                            }                            
                            else
                        ...
                        if (newCdrRating > oldCdrRating)
                        {
                            playerMsg = $"You have successfully used the {source.Name} to upgrade your {target.NameWithMaterial} with Critical Damage Resist Rating {newCdrRating}!";
                        }
                        else if (newCdrRating == oldCdrRating)
                        {
                            playerMsg = $"The {source.Name} has failed to upgrade your {target.NameWithMaterial} with its Critical Damage Resist Rating of {newCdrRating} remaining unchanged.";
                        }
                        else
                        {
                            playerMsg = $"The {source.Name} has failed and has damaged your {target.NameWithMaterial} resulting in a new Critical Damage Resist Rating of {newCdrRating}.";
                        }
                        ...
                        target.GearCritDamageResist = newCdrRating;
                    ...
                    case MorphGemCD:
                        ...
                        if (target as MeleeWeapon == null &&
                            ...
                            !(target.ClothingPriority?.HasFlag(CoverageMask.UnderwearChest) ?? false) &&
                            !(target.ClothingPriority?.HasFlag(CoverageMask.UnderwearUpperLegs) ?? false &&
                            !(target.ItemType == ItemType.Armor || target.ItemType == ItemType.Jewelry || target.ItemType == ItemType.Clothing)))
                        {
                            playerMsg = "This gem can only be used on armor, jewelry, undies, cloaks, casters, 2 hand weapons, missile weapons and shields";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                        ...
                        //Dont allow on 1 hand melee weps
                        ...
                        imbueStackingBitsCleared = false;

                        //Handle armor
                        if ((target.ArmorLevel ?? 0) > 0 && !target.IsShield)
                        {
                            if (target.GearCritDamage >= 3)
                            {
                            ...
                            {
                                newCDRating = ThreadSafeRandom.Next(2, 3);
                            }
                            else if (bracketRoll > 0.99f)
                            {
                                newCDRating = ThreadSafeRandom.Next(1, 3);
                            }
                            else if (bracketRoll > 0.5f)
                            {
                                newCDRating = ThreadSafeRandom.Next(1, 2);
                            }
                            else
                            {
                                newCDRating = 1;
                        ...
                        {
                            playerMsg = $"You have successfully used the {source.Name} to upgrade your {target.NameWithMaterial} with Critical Damage Rating {newCDRating}!";
                        }
                        else if (newCDRating == oldCDRating)
                        {
                            playerMsg = $"The {source.Name} has failed to upgrade your {target.NameWithMaterial} with its Critical Damage Rating of {newCDRating} remaining unchanged.";
                        }
                        else
                        {
                            playerMsg = $"The {source.Name} has failed and has damaged your {target.NameWithMaterial} resulting in a new Critical Damage Rating of {newCDRating}.";
                        }
                    ...
                    case MorphGemAddSlayer:
                        ...
                        playerMsg = $"You have successfully used the {source.Name} to add {target.SlayerCreatureType.ToDisplayString()} Slayer to your {target.NameWithMaterial}!";
                        player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                    ...
                    case MorphGemDmgRating:
                        ...
                        //Can only be applied to melee weapons, casters, missile weapons, cloaks, armor, jewelry and undies
                        if (target as MeleeWeapon == null &&
                            !target.IsCaster &&
                            !target.IsRanged &&
                            !target.IsShield &&
                            ...
                            !(target.ItemType == ItemType.Armor || target.ItemType == ItemType.Jewelry || target.ItemType == ItemType.Clothing)))
                        {
                            playerMsg = "This gem can only be used on loot generated weapons, magic casters, shields, cloaks, armor, jewelry or underwear";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                        ...
                        //For armor or jewelry, roll a random Dmg rating of 1 to 3
                        else if (((target.ItemType == ItemType.Armor || target.ItemType == ItemType.Clothing) && target.ArmorLevel > 0) ||
                            target.ItemType == ItemType.Jewelry)
                        {
                            if (oldDmgRating >= 3)
                            {
                            ...
                            if (dmgBracketRoll > 0.999f)
                            {
                                newDmgRating = 3;
                            }
                            else if (dmgBracketRoll > 0.99f)
                            {
                                newDmgRating = ThreadSafeRandom.Next(2, 3);
                            }
                            else if (dmgBracketRoll > 0.95f)
                            {
                                newDmgRating = ThreadSafeRandom.Next(1, 3);
                            }
                            else if (dmgBracketRoll > 0.5f)
                            {
                                newDmgRating = ThreadSafeRandom.Next(1, 2);
                            }
                            else
                            {
                                newDmgRating = 1;
                        ...
                        {
                            playerMsg = $"You have successfully used the {source.Name} to upgrade your {target.NameWithMaterial} with Damage Rating {newDmgRating}!";
                        }
                        else if (newDmgRating == oldDmgRating)
                        {
                            playerMsg = $"The {source.Name} has failed to upgrade your {target.NameWithMaterial} with its Damage Rating of {newDmgRating} remaining unchanged.";
                        }
                        else
                        {
                            playerMsg = $"The {source.Name} has failed and has damaged your {target.NameWithMaterial} resulting in a new Damage Rating of {newDmgRating}.";
                        }
                    ...
                    case MorphGemDmgResistRating:
                        ...
                        if (!(target.ValidLocations?.HasFlag(EquipMask.Cloak) ?? false) &&
                            ...
                            !(target.ItemType == ItemType.Armor || target.ItemType == ItemType.Jewelry || target.ItemType == ItemType.Clothing))
                        {
                            playerMsg = "This gem can only be used on loot generated armor, jewerly, cloaks or underwear";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                        ...
                        if (!(target.ValidLocations?.HasFlag(EquipMask.Cloak) ?? false) && !target.Workmanship.HasValue)
                        {
                            playerMsg = "This gem can only be used on loot generated armor, jewelry, cloaks or underwear";
                            player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                            player.SendUseDoneEvent(WeenieError.YouDoNotPassCraftingRequirements);
                            return;
                        }
                            ...
                            target.ItemType == ItemType.Jewelry)
                        {
                            ...
                            var dmgResistBracketRoll = ThreadSafeRandom.Next(0.0f, 1.0f);

                            if (dmgResistBracketRoll > 0.999f)
                            {
                                newDmgResistRating = 3;
                            }
                            else if (dmgResistBracketRoll > 0.99f)
                            {
                                newDmgResistRating = ThreadSafeRandom.Next(2, 3);
                            }
                            else if (dmgResistBracketRoll > 0.95f)
                            {
                                newDmgResistRating = ThreadSafeRandom.Next(1, 3);
                            }
                            else if (dmgResistBracketRoll > 0.5f)
                            {
                                newDmgResistRating = ThreadSafeRandom.Next(1, 2);
                            }
                            else
                            {
                                newDmgResistRating = 1;
                            }
                        }
                        ...
                        playerMsg = $"You have successfully used the {source.Name} to upgrade your {target.NameWithMaterial} with Damage Resist Rating {target.GearDamageResist}!";
                        player.Session.Network.EnqueueSend(new GameMessageSystemChat(playerMsg, ChatMessageType.Broadcast));
                    ...
                    case MorphGemRandomizeWeaponElement:
                        ...
                        break;
                    #endregion MorphGemRandomizeWeaponElement
        ...
        private static void UpdateWeaponElementSwap(Player player, WorldObject source, WorldObject target, int wcid)
        {
            ...
            Tailoring.UpdateWeaponProps(player, source, target);
